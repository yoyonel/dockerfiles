...
if grep -Fxq "Host docker-chuck" ~/.ssh/config 
	then
	echo ">> Host docker-chuck déjà présent dans ~/.ssh/config"	
else
	echo ">> On insère la config SSH pour ChucK dans un Docker container"
